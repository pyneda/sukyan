...
var (
	...
	HttpMethodOverrideCode IssueCode = "http_method_override"
	...
	SoapActionSpoofingCode IssueCode = "soap_action_spoofing"
...
var issueTemplates = []IssueTemplate{
	...
	},
	{
		...
		References: []string{
			"https://cheatsheetseries.owasp.org/cheatsheets/GraphQL_Cheat_Sheet.html",
			"https://escape.tech/blog/graphql-security-testing/",
		},
	},
	{
	...
	{
		Code:        HttpMethodOverrideCode,
		Title:       "HTTP Method Override Accepted",
		Description: "The application accepts HTTP method override headers, allowing the actual HTTP\nmethod to be changed via headers or query parameters. This can potentially be\nused to bypass method-based access controls.\n\nCommon override mechanisms:\n- X-HTTP-Method-Override header\n- X-Method-Override header\n- _method query parameter\n\nFor example, a GET request with X-HTTP-Method-Override: DELETE might be\nprocessed as a DELETE request, bypassing security controls that only check\nthe actual HTTP method.",
		Remediation: "Disable HTTP method override functionality in production unless absolutely required.\nIf method override is needed:\n- Implement strict validation of the original method\n- Apply the same authorization checks regardless of override\n- Log method override usage for security monitoring\n- Consider restricting which methods can be overridden",
		Cwe:         650,
		Severity:    "Medium",
		References: []string{
			"https://owasp.org/www-project-web-security-testing-guide/latest/4-Web_Application_Security_Testing/02-Configuration_and_Deployment_Management_Testing/06-Test_HTTP_Methods",
	...
	{
		Code:        SchemaValidationMissingCode,
		Title:       "Schema Validation Missing",
		Description: "The API endpoint does not properly validate input against its defined schema constraints. When testing with values that violate the API specification (such as values outside defined min/max bounds, strings exceeding length limits, or values not matching required patterns), the server accepted the request instead of rejecting it.\n\nThis indicates that the server-side validation does not match the API specification, which can lead to:\n- Data integrity issues when invalid data is stored\n- Business logic bypass when constraints are not enforced\n- Potential security vulnerabilities if malformed input reaches backend systems\n- Inconsistency between documented API behavior and actual implementation\n\nMissing schema validation is particularly concerning for:\n- Numeric bounds (min/max values)\n- String length constraints\n- Pattern/regex validation\n- Enum value restrictions\n- Required field enforcement",
		Remediation: "Implement comprehensive server-side validation that enforces all schema constraints defined in the API specification:\n\n1. Validate all input parameters against their defined types and constraints\n2. Ensure numeric values respect min/max bounds\n3. Enforce string length limits (minLength/maxLength)\n4. Apply pattern validation for formatted fields (email, UUID, etc.)\n5. Restrict values to defined enum lists where applicable\n6. Return appropriate 400 Bad Request responses for invalid input\n7. Never rely solely on client-side validation\n\nConsider using validation libraries that can automatically enforce OpenAPI/JSON Schema constraints.",
		Cwe:         20,
		Severity:    "Medium",
		References: []string{
			"https://owasp.org/www-project-web-security-testing-guide/latest/4-Web_Application_Security_Testing/07-Input_Validation_Testing/",
	...
	{
		Code:        WsdlExposureCode,
		Title:       "WSDL File Publicly Accessible",
		Description: "The WSDL (Web Services Description Language) file is publicly accessible. This\nfile describes the complete structure of the SOAP web service including all\noperations, parameters, data types, and endpoint locations.\n\nThe exposed WSDL provides attackers with:\n- Complete list of available operations\n- Parameter names and types for each operation\n- Data structure definitions\n- Security binding information\n- Endpoint URLs",
		Remediation: "Consider restricting access to WSDL files in production:\n- Require authentication to access the WSDL\n- Use IP-based access controls to limit WSDL access\n- Disable automatic WSDL generation (?wsdl parameter)\n- Host WSDL files in a separate, protected location\n\nIf WSDL must be public, ensure all documented operations are:\n- Properly authenticated\n- Authorized based on user roles\n- Protected against common attacks",
		Cwe:         200,
		Severity:    "Low",
		References: []string{
			"https://owasp.org/www-project-web-security-testing-guide/latest/4-Web_Application_Security_Testing/12-API_Testing/",
