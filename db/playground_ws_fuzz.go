...
type PlaygroundWsFuzzIteration struct {
	BaseModel
	RunID                 uint                 `gorm:"index;uniqueIndex:idx_ws_fuzz_run_iter,priority:1" json:"run_id"`
	Run                   PlaygroundWsFuzzRun  `json:"-" gorm:"foreignKey:RunID;constraint:OnUpdate:CASCADE,OnDelete:CASCADE;"`
	IterationIndex        int                  `gorm:"uniqueIndex:idx_ws_fuzz_run_iter,priority:2" json:"iteration_index"`
	Status                string               `gorm:"index" json:"status"`
