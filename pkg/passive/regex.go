...
var apiKeysPatternsMap = map[string]*regexp.Regexp{
	...
	"Mailgun Public Validation Key":     regexp.MustCompile(`pubkey-[a-f0-9]{32}`),
	"Mailgun Webhook signing key":       regexp.MustCompile(`[a-h0-9]{32}-[a-h0-9]{8}-[a-h0-9]{8}`),
	...
	"Planetscale API Key":               regexp.MustCompile(`pscale_tkn_[a-zA-Z0-9_.-]{43}`),
	"Planetscale OAuth token":           regexp.MustCompile(`pscale_oauth_[a-zA-Z0-9_.-]{32,64}`),
	"Planetscale Password":              regexp.MustCompile(`pscale_pw_[a-zA-Z0-9_.-]{43}`),
