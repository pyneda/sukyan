...
func TestValidateRuleAcceptsKnownCombos(t *testing.T) {
	require.NoError(t, ValidateRule(mkRule(FieldStatusCode, OpEq, 200)))
	require.NoError(t, ValidateRule(mkRule(FieldResponseBody, OpContains, "needle")))
	require.NoError(t, ValidateRule(mkRule(FieldResponseHeaders, OpRegex, "X-.*")))
}

func TestValidateRuleRejectsUnknownField(t *testing.T) {
	require.Error(t, ValidateRule(mkRule("bogus", OpEq, 1)))
}

func TestValidateRuleRejectsInvalidOperatorForField(t *testing.T) {
	require.Error(t, ValidateRule(mkRule(FieldStatusCode, OpContains, "x")))
	require.Error(t, ValidateRule(mkRule(FieldResponseBody, OpEq, "x")))
}
