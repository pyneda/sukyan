...
var proxyHomepageHtml = `
        <html>
					<head>
						<title>Sukyan Proxy</title>
						<style>
					...
					<body>
						<header><h1>Welcome to Sukyan Proxy!</h1></header>
						<main>
							<ul>
									<li><a href="/ca">Download CA</a></li>
									<li><a href="https://sukyan.com/docs">Documentation</a></li>
							</ul>
						</main>
					</body>
        </html>`

var proxyInterceptHtml = `
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <main>
        ...
        <label>URL:</label>
        <input type="text" id="requestURL" size="50"><br><br>

        <label>Method:</label>
        <input type="text" id="requestMethod" size="10"><br><br>

        <label>Headers:</label>
        <div id="headerList">
            <!-- Sample representation of a key-value pair -->
            <!-- <div class="headerPair">
                <input type="text" class="headerKey" value="Sample-Key">
                <input type="text" class="headerValue" value="Sample-Value">
                <button onclick="removeHeaderPair(this)">Remove</button>
            </div> -->
        </div>
        <button onclick="addHeaderPair()">Add Header</button>

        <label>Body:</label>
        <textarea id="requestBody" rows="10" cols="50"></textarea><br><br>
    ...
    <script>
        const ws = new WebSocket('ws://%s/intercept/api');
        ...
        document.getElementById('forwardButton').addEventListener('click', function() {
            ...
            ws.send(JSON.stringify({action: 'forward', request: modifiedRequest}));
        });
        ...
        function addHeaderPair(key = "", value = "") {
            ...
            div.className = 'headerPair';

            const keyInput = document.createElement('input');
            keyInput.type = 'text';
            keyInput.className = 'headerKey';
            keyInput.value = key;

            const valueInput = document.createElement('input');
            valueInput.type = 'text';
            valueInput.className = 'headerValue';
            ...
            document.getElementById('headerList').appendChild(div);
        }
        ...
        function gatherHeaders() {
            const headers = {};
            const pairs = document.querySelectorAll('.headerPair');
            pairs.forEach(pair => {
                const key = pair.querySelector('.headerKey').value.trim();
                const value = pair.querySelector('.headerValue').value.trim();
                if (key && value) {
                    headers[key] = value.split('; ').filter(v => v); // Convert back to an array
                }
            });
            return headers;
        }
...
func generateProxyInterceptHtml(proxyAddress string) string {
	return fmt.Sprintf(proxyInterceptHtml, proxyAddress)
}
