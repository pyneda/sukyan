...
func (s *ScanEngine) FullScan(options scan_options.FullScanOptions, waitCompletion bool) (*db.Task, error) {
	...
	crawler := crawl.NewCrawler(options.StartURLs, options.MaxPagesToCrawl, options.MaxDepth, options.PagesPoolSize, options.ExcludePatterns, options.WorkspaceID, task.ID, 0, 0, options.Headers, options.CaptureBrowserEvents)
	...
	for baseURL, histories := range historiesByBaseURL {
		...
		passive.ReportFingerprints(baseURL, newFingerprints, options.WorkspaceID, task.ID, nil)
